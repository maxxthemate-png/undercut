...
2. **Drive traffic — AUTONOMOUS CHANNELS ONLY.** ⚠️ **HARD CONSTRAINTS (Maxx, 2026-06-10): (a) he has NO eBay account and won't make one; (b) he does ZERO social media and ZERO manual marketing — never assign him forum posts, group posts, DMs, directory submissions, or outreach emails. The machine must market itself.** Growth doctrine: programmatic SEO (registry) + AI-answer-engine surface (llms.txt/llms-full.txt, FAQ schema) + **IndexNow auto-submission** (`scripts/indexnow-ping.mjs` + `.github/workflows/indexnow.yml` — pings Bing/DDG/Yandex on every dashboard push + weekly) + email nurture on captured leads + (when users exist) referral mechanics. Owner-acceptable one-time infra: Search Console sitemap submit, custom domain. `GTM_OUTREACH.md` is an ARCHIVE (manual playbook, unused by choice).
