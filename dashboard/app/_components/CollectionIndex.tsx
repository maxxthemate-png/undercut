    ...
    <div className="min-h-screen bg-paper text-ink">
      ...
      <section className="max-w-3xl mx-auto px-6 pb-14">
        <ul className="space-y-3">
          {pages.map((p) => (
            <li key={p.slug} className="rounded-lg border border-line bg-surface p-4 transition hover:shadow">
              <Link href={`/${collection}/${p.slug}`} className="block">
                <p className="font-semibold text-cut">{p.h1}</p>
                <p className="text-sm text-muted mt-1">{p.metaDescription}</p>
              </Link>
            </li>
          ))}
        </ul>
